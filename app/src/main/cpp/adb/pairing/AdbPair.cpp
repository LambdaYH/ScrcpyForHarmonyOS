...
constexpr int kConnectDiscoveryTimeoutSeconds = 30;
constexpr size_t kExportedKeySize = 64;
constexpr char kAdbPairingServiceType[] = "_adb-tls-pairing._tcp.local";
constexpr char kAdbConnectServiceType[] = "_adb-tls-connect._tcp.local";
...
struct MdnsConnectServiceEntry {
    std::string instanceName;
    std::string address;
    uint16_t port = 0;
};
...
bool EndsWithDnsName(const std::string& name, const std::string& suffix) {
    return name == suffix || (name.size() > suffix.size() && name.compare(name.size() - suffix.size(), suffix.size(), suffix) == 0);
}

void ParseMdnsConnectAnswers(const std::vector<uint8_t>& packet, const std::string& remoteAddress,
                             int preferredFamily, MdnsPairingService& service) {
    constexpr uint16_t kDnsTypeA = 1;
    constexpr uint16_t kDnsTypePtr = 12;
    constexpr uint16_t kDnsTypeSrv = 33;
    constexpr uint16_t kDnsTypeAaaa = 28;
    const std::string serviceType = NormalizeDnsName(kAdbConnectServiceType);
    ...
    for (uint32_t i = 0; i < rrCount; ++i) {
        ...
        const size_t rdataOffset = offset;
        if (type == kDnsTypePtr && name == serviceType) {
            size_t ptrOffset = rdataOffset;
            std::string instanceName;
            if (ReadDnsName(packet, ptrOffset, instanceName)) {
                const std::string normalizedInstance = NormalizeDnsName(instanceName);
                if (EndsWithDnsName(normalizedInstance, serviceType)) {
                    service.instanceName = normalizedInstance;
                }
            }
        } else if (type == kDnsTypeSrv && EndsWithDnsName(name, serviceType) && rdLength >= 6) {
            ...
            if (ReadDnsName(packet, targetOffset, targetHost)) {
                service.targetHost = NormalizeDnsName(targetHost);
                ApplyTargetHostAddress(service, preferredFamily);
                // Some Android devices expose the connect service source IP
                // before the target host A/AAAA record arrives. Keep the
                // packet source as a provisional address so connect discovery
                // can still succeed, while later A/AAAA records may overwrite it.
                if (service.address.empty() && AddressMatchesFamily(remoteAddress, preferredFamily)) {
                    service.address = remoteAddress;
                }
            }
        } else if (type == kDnsTypeA && rdLength == 4) {
...
void AppendUniqueConnectService(std::vector<MdnsConnectServiceEntry>& services, const MdnsConnectServiceEntry& candidate) {
    if (candidate.instanceName.empty() || candidate.address.empty() || candidate.port == 0) {
        return;
    }
    const auto duplicateIt = std::find_if(services.begin(), services.end(), [&](const MdnsConnectServiceEntry& item) {
        return item.instanceName == candidate.instanceName &&
            NormalizeIpLiteral(item.address) == NormalizeIpLiteral(candidate.address) &&
            item.port == candidate.port;
    });
    if (duplicateIt == services.end()) {
        services.push_back(candidate);
    }
}

std::vector<MdnsConnectServiceEntry> ParseMdnsConnectServices(const std::vector<uint8_t>& packet,
                                                              const std::string& remoteAddress,
                                                              int preferredFamily) {
    constexpr uint16_t kDnsTypeSrv = 33;
    const std::string serviceType = NormalizeDnsName(kAdbConnectServiceType);
    std::vector<MdnsConnectServiceEntry> services;
    ...
    const size_t rrCount = static_cast<size_t>(anCount) + nsCount + arCount;
    for (size_t i = 0; i < rrCount; ++i) {
        ...
        const size_t rdataOffset = offset;
        if (type == kDnsTypeSrv && EndsWithDnsName(NormalizeDnsName(name), serviceType) && rdLength >= 6) {
            MdnsConnectServiceEntry candidate;
            candidate.instanceName = NormalizeDnsName(name);
            candidate.port = ReadU16(packet, rdataOffset + 4);
            if (AddressMatchesFamily(remoteAddress, preferredFamily)) {
                candidate.address = remoteAddress;
            }
            AppendUniqueConnectService(services, candidate);
        }
        offset = rdataOffset + rdLength;
    }
    return services;
...
std::vector<std::string> DiscoverAndroidConnectHostPortsIpv4(const std::string& localIp, const std::string& preferredHost);
std::vector<std::string> DiscoverAndroidConnectHostPortsIpv6(const std::string& localIp, const std::string& preferredHost);
std::vector<std::string> DiscoverAndroidConnectHostPorts(const std::string& localIp, const std::string& preferredHost);

std::string DiscoverAndroidConnectHostPortIpv4(const std::string& localIp, const std::string& preferredHost) {
    const auto hostPorts = DiscoverAndroidConnectHostPortsIpv4(localIp, preferredHost);
    if (!hostPorts.empty()) {
        return hostPorts.front();
    }
    throw std::runtime_error("ADB connect service not found");
}

void AppendUniqueHostPort(std::vector<std::string>& hostPorts, const std::string& hostPort) {
    if (std::find(hostPorts.begin(), hostPorts.end(), hostPort) == hostPorts.end()) {
        hostPorts.push_back(hostPort);
    }
}

void PrioritizePreferredConnectHost(std::vector<std::string>& hostPorts, const std::string& preferredHost) {
    if (preferredHost.empty() || hostPorts.size() < 2) {
        return;
    }
    const std::string normalizedPreferredHost = NormalizeIpLiteral(preferredHost);
    auto preferredIt = std::find_if(hostPorts.begin(), hostPorts.end(), [&](const std::string& candidateHostPort) {
        std::string candidateHost;
        std::string candidatePort;
        ParseHostPort(candidateHostPort, candidateHost, candidatePort);
        return NormalizeIpLiteral(candidateHost) == normalizedPreferredHost;
    });
    if (preferredIt != hostPorts.end() && preferredIt != hostPorts.begin()) {
        std::rotate(hostPorts.begin(), preferredIt, preferredIt + 1);
    }
}
...
std::vector<std::string> DiscoverAndroidConnectHostPortsIpv4(const std::string& localIp, const std::string& preferredHost) {
    ...
    UniqueFd fd(::socket(AF_INET, SOCK_DGRAM, 0));
    if (fd.get() < 0) {
        throw std::runtime_error("Failed to create ADB connect discovery socket");
    }
    ...
    bindAddr.sin_addr.s_addr = htonl(INADDR_ANY);
    if (bind(fd.get(), reinterpret_cast<sockaddr*>(&bindAddr), sizeof(bindAddr)) != 0) {
        throw std::runtime_error("Failed to bind ADB connect discovery socket");
    }
    in_addr localAddr {};
    if (inet_pton(AF_INET, localIp.c_str(), &localAddr) == 1) {
        if (setsockopt(fd.get(), IPPROTO_IP, IP_MULTICAST_IF, &localAddr, sizeof(localAddr)) != 0) {
            OH_LOG_WARN(LOG_APP, "ADBConnectDiscovery: set multicast interface failed errno=%{public}d", errno);
        }
    } else {
        OH_LOG_WARN(LOG_APP, "ADBConnectDiscovery: invalid local ip for multicast interface: %{public}s",
                    localIp.c_str());
    }
    ...
    mreq.imr_interface = localAddr;
    if (setsockopt(fd.get(), IPPROTO_IP, IP_ADD_MEMBERSHIP, &mreq, sizeof(mreq)) != 0) {
        OH_LOG_WARN(LOG_APP, "ADBConnectDiscovery: join multicast failed errno=%{public}d", errno);
    }
    ...
    std::vector<std::string> discoveredHostPorts;
    auto deadline = std::chrono::steady_clock::now() + std::chrono::seconds(kConnectDiscoveryTimeoutSeconds);
    int queryCount = 0;
    int responseCount = 0;
    while (std::chrono::steady_clock::now() < deadline) {
        std::vector<std::pair<std::string, uint16_t>> questions {
            {kAdbConnectServiceType, kDnsTypePtr}
        };
        std::vector<uint8_t> query = BuildMdnsQuery(questions);
        const ssize_t sent = sendto(fd.get(), query.data(), query.size(), 0,
                                    reinterpret_cast<sockaddr*>(&multicast), sizeof(multicast));
        if (sent < 0) {
            OH_LOG_WARN(LOG_APP, "ADBConnectDiscovery: mDNS query send failed errno=%{public}d", errno);
        }
        ++queryCount;
        ...
        if (ready > 0 && FD_ISSET(fd.get(), &readSet)) {
            ...
            if (received > 0) {
                ...
                inet_ntop(AF_INET, &remote.sin_addr, remoteAddr, sizeof(remoteAddr));
                ++responseCount;
                const auto services = ParseMdnsConnectServices(packet, remoteAddr, AF_INET);
                for (const auto& service : services) {
                    AppendUniqueHostPort(discoveredHostPorts, FormatHostPort(service.address, service.port));
                }
            }
        } else if (ready < 0 && errno != EINTR) {
            OH_LOG_WARN(LOG_APP, "ADBConnectDiscovery: select failed errno=%{public}d", errno);
        }
    }

    if (!discoveredHostPorts.empty()) {
        PrioritizePreferredConnectHost(discoveredHostPorts, preferredHost);
        return discoveredHostPorts;
    }

    OH_LOG_WARN(LOG_APP,
                "ADBConnectDiscovery: service not found localIp=%{public}s preferredHost=%{public}s queries=%{public}d responses=%{public}d",
                localIp.c_str(), preferredHost.c_str(), queryCount, responseCount);
    throw std::runtime_error("ADB connect service not found");
}

std::string DiscoverAndroidConnectHostPortIpv6(const std::string& localIp, const std::string& preferredHost) {
    const auto hostPorts = DiscoverAndroidConnectHostPortsIpv6(localIp, preferredHost);
    if (!hostPorts.empty()) {
        return hostPorts.front();
    }
    throw std::runtime_error("ADB connect service not found");
...
std::vector<std::string> DiscoverAndroidConnectHostPortsIpv6(const std::string& localIp, const std::string& preferredHost) {
    ...
    const uint32_t interfaceIndex = FindInterfaceIndexForAddress(localIp);
    if (interfaceIndex == 0) {
        throw std::runtime_error("IPv6 mDNS interface not found");
    }
    ...
    UniqueFd fd(::socket(AF_INET6, SOCK_DGRAM, 0));
    if (fd.get() < 0) {
        throw std::runtime_error("Failed to create IPv6 ADB connect discovery socket");
    }
    ...
    bindAddr.sin6_scope_id = interfaceIndex;
    if (bind(fd.get(), reinterpret_cast<sockaddr*>(&bindAddr), sizeof(bindAddr)) != 0) {
        throw std::runtime_error("Failed to bind IPv6 ADB connect discovery socket");
    }

    if (setsockopt(fd.get(), IPPROTO_IPV6, IPV6_MULTICAST_IF, &interfaceIndex, sizeof(interfaceIndex)) != 0) {
        OH_LOG_WARN(LOG_APP, "ADBConnectDiscovery: set IPv6 multicast interface failed errno=%{public}d", errno);
    }
    ...
    mreq.ipv6mr_interface = interfaceIndex;
    if (setsockopt(fd.get(), IPPROTO_IPV6, IPV6_JOIN_GROUP, &mreq, sizeof(mreq)) != 0) {
        OH_LOG_WARN(LOG_APP, "ADBConnectDiscovery: join IPv6 multicast failed errno=%{public}d", errno);
    }
    ...
    while (std::chrono::steady_clock::now() < deadline) {
        ...
        const ssize_t sent = sendto(fd.get(), query.data(), query.size(), 0,
                                    reinterpret_cast<sockaddr*>(&multicast), sizeof(multicast));
        if (sent < 0) {
            OH_LOG_WARN(LOG_APP, "ADBConnectDiscovery: IPv6 mDNS query send failed errno=%{public}d", errno);
        }
        ++queryCount;
        ...
        if (ready > 0 && FD_ISSET(fd.get(), &readSet)) {
            ...
            if (received > 0) {
                ...
                inet_ntop(AF_INET6, &remote.sin6_addr, remoteAddr, sizeof(remoteAddr));
                ++responseCount;
                const auto services = ParseMdnsConnectServices(packet, remoteAddr, AF_INET6);
                for (const auto& service : services) {
                    AppendUniqueHostPort(discoveredHostPorts,
                                         FormatScopedIpv6HostPort(service.address, service.port, interfaceIndex));
                }
            }
        } else if (ready < 0 && errno != EINTR) {
            OH_LOG_WARN(LOG_APP, "ADBConnectDiscovery: IPv6 select failed errno=%{public}d", errno);
        }
    }
    ...
    OH_LOG_WARN(LOG_APP,
                "ADBConnectDiscovery: IPv6 service not found localIp=%{public}s preferredHost=%{public}s queries=%{public}d responses=%{public}d",
                localIp.c_str(), preferredHost.c_str(), queryCount, responseCount);
    throw std::runtime_error("ADB connect service not found");
}

std::string DiscoverAndroidConnectHostPort(const std::string& localIp, const std::string& preferredHost) {
    const auto hostPorts = DiscoverAndroidConnectHostPorts(localIp, preferredHost);
    if (!hostPorts.empty()) {
        return hostPorts.front();
    }
    throw std::runtime_error("ADB connect service not found");
...
std::vector<std::string> DiscoverAndroidConnectHostPorts(const std::string& localIp, const std::string& preferredHost) {
    if (IsIpv6Literal(localIp)) {
        return DiscoverAndroidConnectHostPortsIpv6(localIp, preferredHost);
    }
    return DiscoverAndroidConnectHostPortsIpv4(localIp, preferredHost);
}
...
void RunQrPairingSession(const std::shared_ptr<QrPairingSession>& session) {
    try {
        ...
        try {
            connectHostPort = DiscoverAndroidConnectHostPort(session->localIp, pairedHost);
        } catch (const std::exception& e) {
            OH_LOG_WARN(LOG_APP,
                        "QRPairing: discover connect service failed session=%{public}lld error=%{public}s",
                        static_cast<long long>(session->sessionId), e.what());
        }
        const std::string resultHostPort = connectHostPort;
        ThrowIfCanceled(&session->stopped);
        OH_LOG_INFO(LOG_APP,
                    "QRPairing: Android QR pairing succeeded session=%{public}lld guid=%{public}s pairedHost=%{public}s hostPort=%{public}s",
                    static_cast<long long>(session->sessionId), guid.c_str(), pairedHost.c_str(), resultHostPort.c_str());
        CompleteQrPairingSession(session, guid, pairedHost, resultHostPort, "");
    } catch (const std::exception& e) {
...
std::vector<std::string> DiscoverConnectHostPorts(const std::string& localIp, const std::string& preferredHost) {
    if (localIp.empty()) {
        throw std::runtime_error("ADB connect discovery local ip is empty");
    }
    return DiscoverAndroidConnectHostPorts(localIp, preferredHost);
}
...
std::string DiscoverConnectHostPort(const std::string& localIp, const std::string& preferredHost) {
    ...
    const auto hostPorts = DiscoverConnectHostPorts(localIp, preferredHost);
    if (!hostPorts.empty()) {
        return hostPorts.front();
    }
    throw std::runtime_error("ADB connect service not found");
